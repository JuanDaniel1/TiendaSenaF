...
import 'package:shop_app/models/enviadocomerc_model.dart';
import '../../config.dart';

class APIenviadoComerc {
  static var client = http.Client();

  static Future<List<EnviadoComercModel>?> getenviados() async {
    Map<String, String> requestHeaders = {
      'Content-Type': 'application/json',
    };

    var url = Uri.parse("${Config.apiURL}${Config.enviadosComercAPI}");
    ...
    if (response.statusCode == 200) {
      return compute(enviadocomercFromJson, response.body);
  ...
  static Future<bool> saveenviado(
      EnviadoComercModel model,
      bool isEditMode,
      bool isFileSelected,
      ) async {
    var productURL = "${Config.enviadosComercAPI}/";
